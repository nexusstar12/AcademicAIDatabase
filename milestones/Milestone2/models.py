...
class StudentModel:
  ...
  def get_average_gpa_by_major(major, minimum_students):
      ...
      if result:
          return "\n".join([f"Major: {row['major']}, Average GPA: {row['average_gpa']:.2f}" for row in result])
      else:
  ...
  def enroll_new_student(major):
      ...
      for course_id in mandatory_courses:
          ...
          Database.insert(enrollment_query, (student_details['student_id'], course_id))
  ...
  def generate_student_details(major):
      ...
      return {
          'name': name,
          'major': major,
          'enrollment_year': f"{enrollment_year}-09-01",  # Assuming enrollment in September
          'GPA': gpa
      }
...
class CourseModel:
  ...
  def get_enrollment_report(minimum_students):
      ...
      if result:
          return "\n".join([f"Course: {row['title']}, Enrolled Students: {row['enrolled_students']}" for row in result])
      else:
          return f"No courses found with at least {minimum_students} students enrolled."

  @staticmethod
  def add_new_course(department):
      # Assuming generate_course_details() and get_default_teacher() are defined within this class or imported
      ...
      Database.insert(insert_course_query, course_details)
      ...
      Database.insert(assign_teacher_query, (default_teacher_id, course_details['course_id']))

      return f"New course '{course_details['title']}' added to the {department} department."

  @staticmethod
  def generate_course_details(department):
    # Logic to generate course details
    title = f"Introduction to {department}"
    semester = "Fall 2023"
    credits = 4
...
class EducatorModel:
  ...
  def get_teachers_above_threshold(score_threshold):
      ...
      if result:
          return "\n".join([f"Teacher: {row['name']}, Average Score: {row['average_score']}" for row in result])
      else:
...
class ClassroomModel:
  ...
  def get_classroom_usage(minimum_usage):
      ...
      if result:
          return "\n".join([f"Classroom: {row['name']}, Weekly Usage: {row['weekly_usage_count']} times" for row in result])
      else:
...
class GradeModel:
  ...
  def calculate_new_grades(course_id, criteria):
      ...
      for record in current_grades:
          ...
          if criteria == 'excellent_attendance':
              ...
              numerical_grade = max(numerical_grade - 0.3, 0.0)  # Minimum grade is 0.0
  ...
  def get_letter_grade(numerical_grade):
      ...
      for grade, value in sorted(GradeModel.grade_mapping.items(), key=lambda x: x[1], reverse=True):
...
class CourseRecommendationModel:
  ...
  def recommend_courses(interests, performance):
      ...
      if result:
          recommendations = "\n".join([f"Course: {row['course_name']}" for row in result])
